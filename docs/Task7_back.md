...
**Target Benchmarks:**
- **broken_large:** DCA ≥ 90% (most distinctive defect)
- **broken_small:** DCA ≥ 80% (smaller features, harder)
- **contamination:** DCA ≥ 65% (most ambiguous)
- **good:** DCA ≥ 75% (should be clearly non-defective)
...
**Method:**
1. Extract deep features (e.g., ResNet penultimate layer) for generated samples per class (50-100 recommended)
...
- [ ] Generate 50-100 samples per class at epoch 250
...
- [ ] Compute Defect Classification Accuracy (DCA)
...
# Generate 50-100 samples per class (adjust based on computational budget)
...
### 7.2 Defect Classification Accuracy
...
# Load fine-tuned defect classifier (started from ImageNet pre-trained weights)
classifier = models.resnet50(pretrained=True)  # Load ImageNet weights
classifier.fc = torch.nn.Linear(2048, 4)  # Replace with 4 defect classes
classifier.load_state_dict(torch.load("classifier_real.pth"))  # Load fine-tuned weights
classifier.eval()
...
# Compute DCA per class
for class_id in range(4):
    dca = correct[class_id] / total[class_id] * 100
    print(f"Class {class_id} DCA: {dca:.2f}%")
```
...
features = []
for img in generated_images:  # 50-100 samples per class
    with torch.no_grad():
...
1. **Generate samples:** 50-100 per class at epoch 250 (minimum 50 for stable FID)
...
2. **Compute DCA:** How well classifier recognizes synthetic defects
...
| **Defect Classification Accuracy** | ≥ 75% avg | HIGH |
...
**Solution:** Train custom defect classifier (ResNet-50) for DCA metric
...
2. **Week 1:** Train defect classifier on real data for DCA
3. **Week 2:** Generate 50-100 samples per class + compute all metrics
